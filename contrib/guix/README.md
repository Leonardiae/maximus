...
This directory contains the files necessary to perform bootstrappable Maximus
Core builds.
...
To build Maximus Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Maximus Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/maximus-detached-sigs> ./contrib/guix/guix-codesign
```
