...
const CBlock* AuxpowMiner::getCurrentBlock(const ChainstateManager& chainman,
    ...
    if (pblockCur == nullptr
            ...
            && GetTime() - startTime > 60))
      {
        ...
        std::unique_ptr<CBlockTemplate> newBlock
            = BlockAssembler(*sporkManager, *governance, *node.llmq_ctx, *node.evodb, chainman.ActiveChainstate(), mempool, Params()).CreateNewBlock (scriptPubKey);
        if (newBlock == nullptr)
                             ...
                             const CScript& scriptPubKey)
{
  ...
  uint256 target;
  const CBlock* pblock = getCurrentBlock (chainman, node, scriptPubKey, target);
