...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in maximus.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in maximus.qrc)
...
static const char* qt_argv = "maximus-qt";
...
void BitcoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success)
    {
        ...
        // maximus: URIs or payment requests:
        if (paymentServer) {
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-font-family", QObject::tr("Set the font family. Possible values: %1. (default: %2)").arg("SystemDefault, Merriweather").arg(GUIUtil::fontFamilyToString(GUIUtil::getFontFamilyDefault())).toStdString(), ArgsManager::ALLOW_ANY, OptionsCategory::GUI);
    ...
    argsman.AddArg("-windowtitle=<name>", _("Sets a window title which is appended to \"Maximus Core - \"").translated, ArgsManager::ALLOW_ANY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(maximus);
    Q_INIT_RESOURCE(maximus_locale);
    ...
    /// 6. Determine availability of data directory and parse maximus.conf
    ...
    // maximus: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
    ...
    if (gArgs.IsArgSet("-font-family")) {
        ...
        try {
            ...
            QMessageBox::critical(0, PACKAGE_NAME,
                                  QObject::tr("Error: Specified font-family invalid. Valid values: %1.").arg("SystemDefault, Merriweather"));
            return EXIT_FAILURE;
        }
