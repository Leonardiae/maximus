...
public:
    explicit AmountValidator(QObject *parent) :
        QValidator(parent),
        currentUnit(BitcoinUnits::MAXIMUS) {}
...
public:
    ...
    QSize minimumSizeHint() const override
    {
        ...
        int h = 0;
        int w = GUIUtil::TextWidth(fm, BitcoinUnits::format(BitcoinUnits::MAXIMUS, BitcoinUnits::maxMoney(), false, BitcoinUnits::SeparatorStyle::ALWAYS));
        w += 2; // cursor blinking space
        w += GUIUtil::maximusThemeActive() ? 24 : 0; // counteract padding from css
        return QSize(w, h);
    }

private:
    int currentUnit{BitcoinUnits::MAXIMUS};
