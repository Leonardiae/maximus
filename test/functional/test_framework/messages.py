...
# Objects that map to maximusd objects, which can be serialized/deserialized
...
class CBlockHeader:
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(maximushash(r))
            self.hash = maximushash(r)[::-1].hex()
...
class CompressibleBlockHeader:
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(maximushash(r))
            self.hash = int(maximushash(r)[::-1].hex(), 16)
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in maximusd indicates these should be deserialized as blocks
