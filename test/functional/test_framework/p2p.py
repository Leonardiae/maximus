...
"""Test objects for interacting with a maximusd node over the p2p protocol.

The P2PInterface objects interact with the maximusd nodes under test using the
...
class P2PConnection(asyncio.Protocol):
    ...
    def peer_connect(self, dstaddr, dstport, *, net, timeout_factor, uacomment=None):
        ...
        logger.debug('Connecting to Maximus Node: %s:%d' % (self.dstaddr, self.dstport))
...
class P2PInterface(P2PConnection):
    """A high-level P2P interface class for communicating with a Maximus node.
