...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Send 210 MAXIMUS from 0 to 2 using sendtoaddress call.
        ...
        # node0 should end up with 1000 MAXIMUS in block rewards plus fees, but
        ...
        # Send 100 MAXIMUS with subtract fee from amount
        ...
        # Sendmany 100 MAXIMUS with subtract fee from amount
        ...
        # Sendmany with explicit fee (MAXIMUS/kB)
        ...
        # Sendmany with explicit fee (muff/B)
        # Throw if no conf_target provided
        assert_raises_rpc_error(-8, "Selected estimate_mode requires a fee rate",
            self.nodes[2].sendmany,
            amounts={ address: 10 },
            estimate_mode='muff/b')
        # Throw if negative feerate
        assert_raises_rpc_error(-3, "Amount out of range",
            self.nodes[2].sendmany,
            amounts={ address: 10 },
            ...
            estimate_mode='muff/b')
        fee_muff_per_b = 2
        fee_per_kb = fee_muff_per_b / 100000.0
        explicit_fee_per_byte = Decimal(fee_per_kb) / 1000
        txid = self.nodes[2].sendmany(
            amounts={ address: 10 },
            conf_target=fee_muff_per_b,
            estimate_mode='muff/b',
        )
        self.nodes[2].generate(1)
        self.sync_all(self.nodes[0:3])
        ...
        # This will raise an exception for attempting to get the private key of an invalid Maximus address
        assert_raises_rpc_error(-5, "Invalid Maximus address", self.nodes[0].dumpprivkey, "invalid")

        # This will raise an exception for attempting to set a label for an invalid Maximus address
        assert_raises_rpc_error(-5, "Invalid Maximus address", self.nodes[0].setlabel, "invalid address", "label")

        # This will raise an exception for importing an invalid address
        assert_raises_rpc_error(-5, "Invalid Maximus address or script", self.nodes[0].importaddress, "invalid")
        ...
        # send with explicit maximus/kb fee
        self.log.info("test explicit fee (sendtoaddress as maximus/kb)")
        self.nodes[0].generate(1)
        self.sync_all(self.nodes[0:3])
        ...
        # send with explicit muff/b fee
        self.sync_all(self.nodes[0:3])
        self.log.info("test explicit fee (sendtoaddress as muff/b)")
        self.nodes[0].generate(1)
        prebalance = self.nodes[2].getbalance()
        assert prebalance > 2
        address = self.nodes[1].getnewaddress()
