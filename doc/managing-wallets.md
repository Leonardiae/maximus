...
Since version 0.21, Maximus Core no longer has a default wallet.
...
The following command, for example, creates a descriptor wallet. More information about this command may be found by running `maximus-cli help createwallet`.

```
$ maximus-cli -named createwallet wallet_name="wallet-01" descriptors=true
```
...
| -----------------|:------------------------------------------------------------|
| Linux            | `/home/<user>/.maximuscore/wallets`                             |
| Windows          | `C:\Users\<user>\AppData\Roaming\MaximusCore\wallets`           |
| macOS            | `/Users/<user>/Library/Application Support/MaximusCore/wallets` |
...
```
$ maximus-cli -rpcwallet="wallet-01" encryptwallet "passphrase"
```

Once encrypted, the passphrase can be changed with the `walletpassphrasechange` command.

```
$ maximus-cli -rpcwallet="wallet-01" walletpassphrasechange "oldpassphrase" "newpassphrase"
```
...
```
$ maximus-cli -named createwallet wallet_name="wallet-01" descriptors=true passphrase="passphrase"
```
...
```
$ maximus-cli -rpcwallet="wallet-01" sendtoaddress "tb1qw508d6qejxtdg4y5r3zarvary0c5xw7kxpjzsx" 0.01
error code: -13
error message:
...
```
$ maximus-cli -rpcwallet="wallet-01" walletpassphrase "passphrase" 120
```
...
```
$ maximus-cli -rpcwallet="wallet-01" backupwallet /home/node01/Backups/backup-01.dat
```
...
```
$ maximus-cli restorewallet "restored-wallet" /home/node01/Backups/backup-01.dat
```

After that, `getwalletinfo` can be used to check if the wallet has been fully restored.

```
$ maximus-cli -rpcwallet="restored-wallet" getwalletinfo
```
