# Fuzzing Maximus Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Maximus Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):
...
# libFuzzer" on https://github.com/maximus-chain/maximus/blob/develop/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[`process_message`](https://github.com/maximus-chain/maximus/blob/develop/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/maximus-chain/maximus/blob/develop/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/maximus-chain/maximus/tree/develop/src/test/fuzz).
...
Every single pull request submitted against the Maximus Core repo is automatically tested against all inputs in the [`bitcoin-core/qa-assets`](https://github.com/bitcoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Maximus Core more robust.
...
with certain assembly code from Maximus Core's code. See [developer notes on sanitizers](https://github.com/maximus-chain/maximus/blob/develop/doc/developer-notes.md#sanitizers)
...
# Fuzzing Maximus Core using afl++

## Quickstart guide

To quickly get started fuzzing Maximus Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Maximus Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Maximus Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the Maximus Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Maximus
Core without having to write any custom fuzzing harness. The `maximusd` server
...
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/maximusd -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
