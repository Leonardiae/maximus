...
The data directory is the default location where the Maximus Core files are stored.
...
3. All content of the data directory, except for `maximus.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Maximus blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
`./`               | `maximus.conf`        | User-defined [configuration settings](maximus-conf.md) for `maximusd` or `maximus-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `maximusd.pid`        | Stores the process ID (PID) of `maximusd` or `maximus-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `maximusd` or `maximus-qt`; can be specified by `-debuglogfile` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [maximus.conf](maximus-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
`maximus-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by Maximus Core:
