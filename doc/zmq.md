...
The Maximus Core daemon can be configured to act as a trusted "border
router", implementing the maximus wire protocol and relay, making
...
The ZeroMQ feature in Maximus Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building maximusd:
...
For instance:
    ...
    $ maximusd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/maximusd.tx.raw \
...
These options can also be provided in maximus.conf.
...
From the perspective of maximusd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into maximusd directly. Furthermore, no information is
...
using. Maximusd appends an up-counting sequence number to each
