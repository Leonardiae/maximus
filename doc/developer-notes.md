...
- [Development guidelines](#development-guidelines)
    - [General Maximus Core](#general-maximus-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Maximus Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
Maximus Core is a multi-threaded application, and deadlocks or other
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_maximus
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_maximus --log_level=test_suite
$ valgrind -v --leak-check=full src/maximusd -printtoconsole
...
# A coverage report will now be accessible at `./test_maximus.coverage/index.html`.
```
...
To profile a running maximusd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep maximusd` -- sleep 60
```
...
Maximus Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`maximusd`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
...
However, in open source software such as Maximus Core, where everyone uses
...
pay attention to for reviewers of Maximus Core code.

General Maximus Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Maximus Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
be sent upstream, but bugfixes may also be prudent to PR against Maximus Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `maximus-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `maximus-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Maximus Core, the wallet was always
...
- Use *invalid* Maximus addresses (e.g. in the constant array `EXAMPLE_ADDRESS`) for
